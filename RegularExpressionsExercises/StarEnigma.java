...
public class StarEnigma {
    public static void main(String[] args) {
        ...
        String messageRegex = "^[^@\\-!:>]*@(?<planetName>[A-Za-z]+)[^@\\-!:>]*" +
                ":(?<population>\\d+)[^@\\-!:>]*!(?<attackType>[AD])![^@\\-!:>]*" +
                "->(?<soldierCount>\\d+)[^@\\-!:>]*";
        ...
        int messCount = Integer.parseInt(scanner.nextLine());
        for (int i = 0; i < messCount; i++) {
            ...
            while (messageMatcher.find()) {
                String planetName = messageMatcher.group("planetName");
